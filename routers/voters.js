...
router.post("/import", [auth, admin], async (req, res) => {
  if (req.files) {
    ...
    if (path.extname(filename) != ".xlsx" && path.extname(filename) != ".csv")
      return res
        .status(400)
        .send({ message: "Invalid file type. Please use a .xlsx file." });

    file.mv("./uploads/" + filename, error => {
      if (error) return res.status(400).send({ message: error });
      else
        return res.send({
          result: ImportExcel(filename),
...
router.post("/searchVoter/", async (req, res) => {
  const findArg = req.body.param;

  for (let k in req.body.param) {
    switch (k) {
      case "fullName":
        findArg.fullName = {
          $regex: new RegExp(".*" + findArg.fullName + ".*"),
          $options: "i"
        };
        break;
      case "section":
        findArg.section = {
          $regex: new RegExp(".*" + findArg.section + ".*"),
          $options: "i"
        };
        break;
    }
  ...
  const voter = await Voter.find(findArg)
    .populate("voteReceiptID")
    .catch(error => {
...
router.post("/auth", async (req, res) => {
  ...
  if (!voter.canVote)
    return res.status(403).send({
      message: "Sorry, voter cannot vote yet. Contact admin for more info."
    });
...
function ImportExcel(filename) {
  ...
  excelToJsonResult.Sheet1.forEach(async element => {
    console.log(element);
    if (!validate(element)) return console.error(`Voter Schema invalid.`);
    ...
    voter = new Voter(
      _.pick(element, ["lrn", "fullName", "gradeLevel", "section"])
    );
